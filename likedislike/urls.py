...
urlpatterns = [   
    ...
    re_path(r'^quote/(?P<pk>\d+)/like/$',
        views.VotesView.as_view(model=Quote, vote_type=LikeDislike.LIKE),
        name='article_like'),
    re_path(r'^quote/(?P<pk>\d+)/dislike/$',
        views.VotesView.as_view(model=Quote, vote_type=LikeDislike.DISLIKE),
        name='article_dislike'),
    re_path(r'^book/(?P<pk>\d+)/like/$',
        views.VotesView.as_view(model=Book, vote_type=LikeDislike.LIKE),
        name='article_like'),
    re_path(r'^book/(?P<pk>\d+)/dislike/$',
        views.VotesView.as_view(model=Book, vote_type=LikeDislike.DISLIKE),
        name='article_dislike'),
]